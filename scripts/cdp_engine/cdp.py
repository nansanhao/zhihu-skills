...
from cdp_engine.errors import CDPError, ElementNotFoundError
from cdp_engine.stealth import STEALTH_JS, build_ua_override
...
class CDPClient:
    ...
    def __init__(self, ws_url: str) -> None:
        ...
        self._id = 0
        self._session_id_counter = 1000  # 供所有 Page 共享的 session 消息 ID
        self._pending: dict[int, dict] = {}  # 缓存已收到但尚未被等待方消费的响应

    def next_session_id(self) -> int:
        """分配下一个 session 消息 ID（线程不安全，当前单线程使用）。"""
    ...
    def send(self, method: str, params: dict | None = None) -> dict:
        ...
        return self.wait_for_id(self._id)

    def wait_for_id(self, msg_id: int, timeout: float = 60.0) -> dict:
        """等待指定 id 的响应，不匹配的响应存入 _pending 供后续消费。"""
        # 先检查 pending 中是否已经有了
        if msg_id in self._pending:
            return self._resolve(self._pending.pop(msg_id))
...
class Page:
    ...
    def _send_session(self, method: str, params: dict | None = None) -> dict:
        """向 session 发送命令。"""
        msg_id = self._cdp.next_session_id()
        msg: dict[str, Any] = {
            "id": msg_id,
            "method": method,
        ...
        if params:
            msg["params"] = params
        self._ws.send(json.dumps(msg))
        return self._cdp.wait_for_id(msg_id)
    ...
    def select_all_and_delete(self) -> None:
        """全选并删除当前焦点元素的内容（Ctrl+A + Backspace）。"""
        self._send_session(
            "Input.dispatchKeyEvent",
    ...
    def dispatch_wheel_event(self, delta_y: float) -> None:
        ...
        self.evaluate(
            f"""
            (() => {{
                let target = document.querySelector('.interaction-container')
