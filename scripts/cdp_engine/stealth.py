...
def _make_platform_config(chrome_ver: str, chrome_full_ver: str) -> dict:
    """根据实际操作系统和指定版本号生成一致的 UA / Client Hints / WebGL 配置。"""
...
PLATFORM_CONFIG = _make_platform_config(_CHROME_VER, _CHROME_FULL_VER)
...
def build_ua_override(chrome_full_ver: str | None = None) -> dict:
    ...
    """
    if chrome_full_ver is None:
        config = PLATFORM_CONFIG
    else:
        major = chrome_full_ver.split(".")[0]
        config = _make_platform_config(major, chrome_full_ver)

    return {
        "userAgent": config["ua"],
        "platform": config["nav_platform"],
        "userAgentMetadata": config["ua_metadata"],
    }
...
(() => {
    ...
    // 8. WebGL vendor/renderer — 与平台一致（同时覆盖 WebGL1 和 WebGL2）
    ...
    // 9. hardwareConcurrency — 随机 4 或 8
    Object.defineProperty(navigator, 'hardwareConcurrency', {
        get: () => [4, 8][Math.floor(Math.random() * 2)],
        configurable: true,
    });

    // 10. deviceMemory — 随机 4 或 8
    Object.defineProperty(navigator, 'deviceMemory', {
        get: () => [4, 8][Math.floor(Math.random() * 2)],
        configurable: true,
    });

    // 11. navigator.connection — 伪造网络信息
    ...
    // 12. chrome.csi / chrome.loadTimes — 空函数伪装
    if (window.chrome) {
        window.chrome.csi = function() { return {}; };
        window.chrome.loadTimes = function() { return {}; };
    }

    // 13. outerWidth/outerHeight — 不覆盖
